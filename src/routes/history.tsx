      ...
      <PageShell title="History">
        <div className="grid grid-cols-3 gap-3 mb-6">
          {[
            ...
            <motion.div key={stat.label} initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: i * 0.05 }} className="glass-panel-elevated p-4 text-center">
        ...
        <div className="glass-panel-elevated overflow-hidden">
          {tripHistory.map((trip, i) => (
            <motion.div key={trip.id} initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: i * 0.04 }} className="flex items-center gap-4 px-5 py-4 border-b border-border last:border-0 hover:bg-surface/50 transition-colors">
