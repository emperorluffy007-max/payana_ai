  ...
  return (
    ...
    >
      ...
      <div className="glass-panel-elevated p-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className={`w-2 h-2 rounded-full ${isActive ? 'bg-hyper-green pulse-green' : 'bg-muted-foreground/40'}`} />
            <span className="text-xs font-medium">{isActive ? 'Transit Mode Active' : 'Standby'}</span>
          </div>
      ...
      <div className="glass-panel-elevated p-3">
        ...
        <p className="metric-value text-xl mt-1">{userStats.co2Saved}<span className="text-xs text-muted-foreground ml-1">kg</span></p>
      </div>
    </motion.div>
  );
}
