import { MapPin, Navigation, Bus, Train, Leaf, ArrowRight, User, Activity, ShieldCheck, AlertTriangle, Clock, Map } from "lucide-react";
...
export function JourneyPanel({ 
  ...
  return (
    <div className="w-[320px] bg-card/80 backdrop-blur-xl border border-border rounded-3xl shadow-xl flex flex-col pointer-events-auto h-full max-h-[85vh] overflow-hidden">
      {/* Header */}
      <div className="p-5 border-b border-white/5 relative shrink-0 transition-colors duration-300" style={{ backgroundColor: isNavigating ? 'rgba(79,70,229,0.1)' : 'transparent' }}>
        <Link to="/plan" className="absolute top-5 right-5 text-indigo text-xs font-semibold uppercase tracking-wider hover:underline">
          Cancel
        </Link>
        <h3 className="font-heading font-medium text-lg mb-1">{isNavigating ? 'Navigating...' : 'Active Journey'}</h3>
        <p className="text-sm text-white/50">{isNavigating ? 'Turn-by-turn guidance active' : 'Tracking live progress'}</p>
      </div>
      ...
      <div className="p-5 flex-1 overflow-y-auto">
          ...
          <div className="relative isolate mb-8">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-indigo mb-0.5 font-medium uppercase tracking-widest">Start</p>
                  <p className="text-sm font-medium">{from}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-6 flex justify-center shrink-0">
                  <MapPin size={16} strokeWidth={2.5} className="text-destructive fill-destructive/20 z-10 bg-card rounded-sm" />
                </div>
                <div>
                  <p className="text-xs text-destructive mb-0.5 font-medium uppercase tracking-widest">Destination</p>
            ...
            <div className="space-y-5">
              ...
              >
                ...
                <div className="flex-1">
                  <p className="text-[10px] text-amber-500 mb-0.5 font-bold uppercase tracking-widest transition-colors group-hover:text-amber-400">Live Bus Location</p>
                  ...
                  {activeCheckpoint === 1 && (
                    <div className="mt-2 text-xs text-white/70 bg-amber-500/10 p-2 rounded-lg border border-amber-500/20 animate-in fade-in slide-in-from-top-2">
                      <p><strong>Status:</strong> On Time (ETA 5 mins)</p>
                      <p><strong>Crowd:</strong> Lightly crowded 🟢</p>
                      <p><strong>Vehicle:</strong> KA-01-F-1234 (Electric)</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              >
                ...
                <div className="flex-1">
                  <p className="text-[10px] text-cyan-400 mb-0.5 font-bold uppercase tracking-widest transition-colors group-hover:text-cyan-300">Walking Distance</p>
                  ...
                  {activeCheckpoint === 2 && (
                    <div className="mt-2 text-xs text-white/70 bg-cyan-500/10 p-2 rounded-lg border border-cyan-500/20 animate-in fade-in slide-in-from-top-2">
                      <p><strong>Estimated Walk:</strong> 4 mins</p>
              ...
              >
                ...
                <div className="flex-1">
                  <p className="text-[10px] text-indigo mb-0.5 font-bold uppercase tracking-widest transition-colors group-hover:text-indigo-400">Remaining Journey</p>
                  ...
                  {activeCheckpoint === 3 && (
                    <div className="mt-2 text-xs text-white/70 bg-indigo/10 p-2 rounded-lg border border-indigo/20 animate-in fade-in slide-in-from-top-2">
                      <p><strong>Overall ETA:</strong> 11:30 AM</p>
                      <p><strong>Traffic:</strong> Moderate at Silk Board Junction 🟠</p>
                      <p><strong>Saved:</strong> 1.2kg CO2 vs driving.</p>
                    </div>
                  )}
                </div>
              </div>
        ...
        <div className="border border-indigo/20 bg-indigo/5 rounded-2xl p-4 mb-4">
          <div className="flex justify-between items-start mb-2">
            <span className="text-xs font-semibold text-indigo tracking-wider uppercase">{mode || 'Journey'} Mode</span>
            <span className="text-xs bg-hyper-green/20 text-hyper-green px-2 py-0.5 rounded-full font-medium flex items-center gap-1"><Leaf size={10} /> Eco-friendly</span>
          </div>
          <div className="text-2xl font-bold font-heading mb-1 text-white">45 min <span className="text-sm font-normal text-white/50 inline-block ml-1">remaining</span></div>
        ...
        {isNavigating && (
          <div className="mt-4 mb-2 p-4 bg-hyper-green/10 border border-hyper-green/30 rounded-xl">
            <p className="text-[10px] text-hyper-green uppercase tracking-widest font-bold mb-1.5 flex items-center gap-1.5"><Activity size={12}/> Action Required</p>
            <p className="text-sm font-medium">Head north on 100ft Road and walk 400m to the nearest Metro Station.</p>
          </div>
        )}
        ...
        {hasMissed && (
          <div className="mb-4 p-4 bg-destructive/10 border border-destructive/30 rounded-xl animate-in fade-in zoom-in duration-300">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-bold text-destructive uppercase tracking-widest text-xs">Bus Missed!</span>
            </div>
        ...
        {!hasMissed ? (
          ...
          <div className="space-y-3 mt-4">
            ...
            >
              ...
              <span className="text-[10px] opacity-70 font-normal">Walk 5m to Nagavara Metro · Save 8m</span>
            </button>
            ...
            >
              ...
              <span className="text-[10px] opacity-70 font-normal">Alternative stop 200m away · Departs 2m</span>
            </button>
